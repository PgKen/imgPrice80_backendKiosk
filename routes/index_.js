...
sqlImgPrice_web += "tb_product.name_pro_en,tb_proselect.price_pro,tb_priceproduct.date_check,"
sqlImgPrice_web += "tb_product.name_pro_cn,picture.name_img,picture.id_pro,tb_unit.id_unit,tb_unit.unitname "
sqlImgPrice_web += "FROM tb_priceproduct "
sqlImgPrice_web += "INNER JOIN tb_proselect ON tb_priceproduct.id_pricepro = tb_proselect.idprice "
sqlImgPrice_web += "INNER JOIN tb_product ON tb_proselect.id_prod = tb_product.id_product "
sqlImgPrice_web += "INNER JOIN tb_typeproduct ON tb_product.typepro = tb_typeproduct.id_typepro "
sqlImgPrice_web += "INNER JOIN picture ON tb_proselect.id_prod = picture.id_pro "
sqlImgPrice_web += "INNER JOIN tb_unit ON tb_proselect.unit = tb_unit.id_unit "
/*WHERE id_mtype = 8*/
/*GROUP BY
tb_proselect.id_prod*/
sqlImgPrice_web += "ORDER BY tb_priceproduct.date_check DESC, RAND() ASC LIMIT 0, 20 "
...
sqlDateLast_web += "tb_product.name_pro_en,tb_proselect.price_pro,tb_priceproduct.date_check,"
sqlDateLast_web += "tb_product.name_pro_cn,picture.name_img,picture.id_pro,tb_unit.id_unit,tb_unit.unitname "
sqlDateLast_web += "FROM tb_priceproduct "
sqlDateLast_web += "INNER JOIN tb_proselect ON tb_priceproduct.id_pricepro = tb_proselect.idprice "
sqlDateLast_web += "INNER JOIN tb_product ON tb_proselect.id_prod = tb_product.id_product "
sqlDateLast_web += "INNER JOIN tb_typeproduct ON tb_product.typepro = tb_typeproduct.id_typepro "
sqlDateLast_web += "INNER JOIN picture ON tb_proselect.id_prod = picture.id_pro "
sqlDateLast_web += "INNER JOIN tb_unit ON tb_proselect.unit = tb_unit.id_unit "
sqlDateLast_web += "WHERE date_check = ? "
sqlDateLast_web += "ORDER BY tb_priceproduct.date_check DESC, RAND() ASC LIMIT 0, 20 "

let sqlImgPrice2_web = "SELECT tb_proselect.id_prod AS rId,tb_product.name_pro,tb_product.name_pro_en,"
sqlImgPrice2_web += "tb_proselect.price_pro,tb_priceproduct.date_check,tb_product.name_pro_cn,picture.name_img,"
sqlImgPrice2_web += "picture.id_pro,tb_unit.id_unit,tb_unit.unitname "
sqlImgPrice2_web += "FROM tb_priceproduct "
sqlImgPrice2_web += "INNER JOIN tb_proselect ON tb_priceproduct.id_pricepro = tb_proselect.idprice "
sqlImgPrice2_web += "INNER JOIN tb_product ON tb_proselect.id_prod = tb_product.id_product "
sqlImgPrice2_web += "INNER JOIN tb_typeproduct ON tb_product.typepro = tb_typeproduct.id_typepro "
sqlImgPrice2_web += "INNER JOIN picture ON tb_proselect.id_prod = picture.id_pro "
sqlImgPrice2_web += "INNER JOIN tb_unit ON tb_proselect.unit = tb_unit.id_unit "
...
/* GET home page. */
router.get('/imgPrice_web', function (req, res, next) {
...
router.get('/delDisplay/:id',(req,res)=>{
  let id = req.params.id
  res.send("id = "+id)
})


router.get('/editDuration',(req,res)=>{
  console.log("edit");
  res.end()
})
...
router.post('/kiosk-upload', function (req, res) {
  ...
  if (!req.files || Object.keys(req.files).length === 0) {
    ...
    // Use the mv() method to place the file somewhere on your server
    sampleFile.mv('./public/kiosk/' + namePic, function (err) {
      if (err)
        return res.status(500).send(err);
      // conn.query(sql, [idProduct, namePic, 1], (err, resIns) => {
      //   if (err) throw console.log(err);
